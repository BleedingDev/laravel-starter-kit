...
const getQrCodeDataUrl = (qrCodeSvg: string) =>
...
const QrCodePanel = ({ qrCodeSvg }: { qrCodeSvg: string | null }) => (
  <div className="mx-auto flex max-w-md overflow-hidden">
    <div className="mx-auto aspect-square w-64 rounded-lg border border-border">
      <div className="z-10 flex h-full w-full items-center justify-center p-5">
        {qrCodeSvg ? (
          <img
            src={getQrCodeDataUrl(qrCodeSvg)}
...
const TwoFactorSetupStep = ({
  ...
  return (
    <>
      <QrCodePanel qrCodeSvg={qrCodeSvg} />
      <SetupStepActions buttonText={buttonText} onNextStep={onNextStep} />
      <ManualSetupDivider />
      <ManualSetupKeyPanel
        manualSetupKey={manualSetupKey}
        onCopy={copy}
...
const OtpSlots = () => (
  <InputOTPGroup>
    {OTP_SLOTS.map((slot) => (
      <InputOTPSlot key={`otp-slot-${slot}`} index={slot - 1} />
    ))}
  </InputOTPGroup>
);
...
const VerificationActions = ({
  ...
  <div className="flex w-full space-x-5">
    ...
    <Button type="submit" className="flex-1" disabled={!canConfirm}>
      Confirm
    </Button>
  </div>
);
      ...
      {({
        ...
        <div ref={pinInputContainerRef} className="relative w-full space-y-3">
          ...
          <VerificationActions
            onBack={onBack}
            disabled={processing}
            canConfirm={!processing && code.length >= OTP_MAX_LENGTH}
          />
        </div>
      )}
    </Form>
